...
// common message variable and frame
		...
		)
{
	...
	msgToSend.nodeType = local_nodeType;

	// initialize TX frame
	frameToSend.frameCtrl[0] = 0x41;
	frameToSend.frameCtrl[1] = 0x88;
	frameToSend.seqNum = 0;
	frameToSend.panId[0] = local_panId[0];
	frameToSend.panId[1] = local_panId[1];
	frameToSend.sourceAddr[0] = local_shortAddress[0];
	frameToSend.sourceAddr[1] = local_shortAddress[1];
				...
				switch( msg->msgType )
				{
					case MSG_RANGE_INIT:
						...
						if( local_nodeType == NODE_TYPE_FIXED || local_nodeType == NODE_TYPE_MOBILE_RSP )
						{
						    ...
						    uint32_t tx_time = tx_time_min & DWR_TIMER_HI_31_BITS;
					...
					case MSG_RANGE_FINAL:
						...
						//dist_est_last.rangeEst = (int)( (ToF_ps - TOF_OFFSET_CONSTANT_PS)*SPEED_OF_LIGHT_CM_PER_PS );
						//dist_est_last.rangeEst = (int)( tround1_us );
						dist_est_last.rangeEst = (int)( ToF_us*SPEED_OF_LIGHT_CM_PER_US );
...
static void dw_send_range_init()
{
	...
	msgToSend.msgType  = MSG_RANGE_INIT;
...
static void dw_send_range_resp(uint8_t* destAddr, uint32_t txtime)
{
	...
	msgToSend.msgType  = MSG_RANGE_RESP;
	msgToSend.rangeSeq = current_rangeSeq;
...
{
	// create range response message
	msgToSend.msgType  = MSG_RANGE_FINAL;
	msgToSend.rangeSeq = current_rangeSeq;
	msgToSend.t_tx[0] = 0x00;
	msgToSend.t_tx[1] = (uint8_t)(txtime);
	msgToSend.t_tx[2] = (uint8_t)(txtime>>8);
	msgToSend.t_tx[3] = (uint8_t)(txtime>>16);
	msgToSend.t_tx[4] = (uint8_t)(txtime>>24);
	msgToSend.t_rx[0] = (uint8_t)(rxtime);
	msgToSend.t_rx[1] = (uint8_t)(rxtime>>8);
	msgToSend.t_rx[2] = (uint8_t)(rxtime>>16);
	msgToSend.t_rx[3] = (uint8_t)(rxtime>>24);
	msgToSend.t_rx[4] = (uint8_t)(rxtime>>32);
...
static void dw_send_ieee_data_delayed(uint8_t* destAddr, DWR_MsgData_t *msg, uint32_t txtime)
{
	...
	// data frame header (dest=short, source=short [DsSs])
	frameToSend.seqNum = local_frame_seqNum++;
