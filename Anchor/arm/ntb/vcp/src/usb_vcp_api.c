...
{
  int msg_idx = VCP_MessageBufferTail;
  int byte_idx = 0;

  while( VCP_NumQueuedMessages > 0 )
  {
    byte_idx = 0;
    while( VCP_MessageBuffer[msg_idx][byte_idx] != '\0' && byte_idx < MAX_VCP_MESSAGE_SIZE )
    {

      VCP_TxByte(VCP_MessageBuffer[msg_idx][byte_idx++]);
    }
    ...
    msg_idx = (VCP_MessageBufferTail + 1) % MAX_VCP_MESSAGES;
...
// Implementation of itoa()
int num2str(int num, uint8_t* str, int base)
{
    int i = 0;
    bool isNegative = false;
