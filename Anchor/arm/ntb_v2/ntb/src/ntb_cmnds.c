...
void ntb_processCmd(uint8_t *raw_cmd, uint8_t *stream_opts)
{
	...
	switch(cmd->sys)
	{
		...
		case NTB_SYS_PMEAS:
			...
			switch(cmd->cmd)
			{
				case NTB_PMEAS_STARTSTREAM:
					(*stream_opts) |= 0x02;
					break;
				case NTB_PMEAS_STOPSTREAM:
					(*stream_opts) &= ~(0x02);
					break;
				default:
					break;
		...
		case NTB_SYS_RANGE:
			...
			switch(cmd->cmd)
			{
				...
				case NTB_RANGE_STOPSTREAM:
					(*stream_opts) &= ~(0x01);
					break;
				case NTB_RANGE_SENDBEACONS:
					// scale to ms and call function
					SetBeaconDelay( (cmd->opt)*10 );
					break;
				case NTB_RANGE_SENDTWR:
					// scale to ms and call function
					SetTwrDelay( (cmd->opt)*10 );
					break;
				case NTB_RANGE_SENDTWR_1SHT:
					// send one TWR message
					sendTwrOneShot();
				default:
					break;
			}
