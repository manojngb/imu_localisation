...
void dw_init()
{
	...
	// decaranging options: always listen, respond to TWR init, send range summaries
	...
	DWR_Config_t dwr_config = {
		...
		.cbBeaconReceived = NULL,
		.cbDebug = dw_debug_handler
	};

	// initialize DWR library
	decaranging_init( dwr_config );

  	// increase SPI frequency to accomodate short turnaround times
  	dw_spi_configprescaler(16); // was 16
	...
	xTaskCreate(rangingTask, (const signed char * const)"Ranging",
		(1 * configMINIMAL_STACK_SIZE), NULL, /*priority*/3, NULL);
}

void rangingTask(void *param)
{
	uint32_t lastWakeTime, lastTwrTime;
	...
	lastTwrTime = xTaskGetTickCount ();

	while(1)
	{
		/* Do nothing for now, just keep task alive */
		vTaskDelayUntil(&lastWakeTime, F2T(20));

		// check if we need to reset transceiver
		if( xTaskGetTickCount() - lastTwrTime > 100 )
		{
...
{
	// toggle the butterlfy red led
	dwt_ntbled_toggle_red();

	/*
	// at the end of each range completion, schedule a TWR to be sent
	if( timeStats->srcAddr == 7 || timeStats->srcAddr == 2 || timeStats->srcAddr == 5)
		twr_queued = 1;
	*/
}
